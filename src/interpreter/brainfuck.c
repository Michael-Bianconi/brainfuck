...
struct Instruction* optimize(char* source) {
  
  long sourcelen = strlen(source);
  struct Instruction* result = calloc(sizeof(struct Instruction), sourcelen);
  int result_idx = 0;
  
  for (int i = 0; i < sourcelen; i++) {
    if (source[i] == '+') {
      result[result_idx++] = (struct Instruction){ADD, 1};
    } else if (source[i] == '-') {
      result[result_idx++] = (struct Instruction){SUB, 1};
    } else if (source[i] == '<') {
      result[result_idx++] = (struct Instruction){LEFT, 1};
    } else if (source[i] == '>') {
      result[result_idx++] = (struct Instruction){RIGHT, 1};
    } else if (source[i] == ',') {
      result[result_idx++] = (struct Instruction){IN, 1};
    } else if (source[i] == '.') {
      result[result_idx++] = (struct Instruction){OUT, 1};
    } else if (source[i] == '[') {
      result[result_idx++] = (struct Instruction){OPEN, 1};
    } else if (source[i] == ']') {
      result[result_idx++] = (struct Instruction){CLOSE, 1};
    }
  }
  
  if (verbose) {
    printf("Skipped %ld no-ops\n", sourcelen - result_idx);
  }
  return result;
}
...
int main(int argc, char* argv[]) {
  ...
  while ((opt = getopt(argc, argv, "x:h:v")) != -1) {
      switch (opt) {
        case 'p':
          sourcefile = optarg;
          break;
